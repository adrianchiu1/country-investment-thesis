...
`prompts/Ada prompt - update an economy.txt` is a prompt for the firm's internal chatbot, Ada; `prompts/READ ME FIRST.txt` walks a researcher through the steps in plain language, and the **Ada prompt** button in the page (next to Import) shows the same steps and copies the prompt to the clipboard. The researcher pastes the prompt into a new Ada conversation with the current `data/<economy>.yaml` attached (Ada asks for the file, and where to find it, if it is missing). Ada then asks the researcher to choose a mode. **Review Mode**: Ada reviews the file with the researcher three items at a time, page by page (Drivers, Signposts, Implications), each item one issue, signpost or matrix cell, and researches only what the researcher picks. **Collaborate Mode**: the researcher sets the agenda and Ada works with them as a junior macro strategist, framing the issue, challenging with numbers and floating decisions, refining wording only when the discussion converges. **Summarise Mode**: the researcher provides a document (their own note, meeting notes, a report); Ada summarises it, extracts numbered key points, and proposes changes grounded in those points alone, with the supporting passage quoted against each one. In every mode Ada agrees the exact wording of every change before recording it, and matrix changes are never written to the file; they go into the change note as suggestions. At the end Ada hands back the whole file with a `change_note` at the top: as a downloadable `<economy>.yaml` where the portal lets Ada attach files, and always as a code block.
