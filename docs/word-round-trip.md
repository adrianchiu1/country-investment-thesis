...
Two prototypes sit next to this note, both generated by `node poc/build.mjs`
from `poc/src/` and the repo's own `data/*.yaml`:

- `poc/docx-round-trip.html` — runs the test suite on load;
- `poc/word-round-trip.html` — the document and the importer's verdict side by
  side, and **drop in a `.docx` that Word has saved** to see what Word did to it.

`poc/sample-united-states-v2.docx` is a document to open in Word.
`word-import-test-protocol.md` is the list of tests to run before the import is
built, and what each result decides.
